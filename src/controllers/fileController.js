...
const uploadUserFile = async (req, res) => {
  try {
    ...
    if (category == "loan") {
      ...
      filename = `${username}_membership_${Date.now()}${extension}`;
    }
...
const AdminUploadFile = async (req, res) => {
  try {
    ...
    if (category == "loan") {
      ...
      filename = `Abacus_membership_form_${fileDate.getFullYear()}_${randomNumber()}${extension}`;
    }
